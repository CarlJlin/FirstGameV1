...
simulate_game(Input* input, float dt) {
	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		if (!ennemy_is_ai) {
			if (is_down(BUTTON_UP)) player_1_ddp += 1700;
			if (is_down(BUTTON_DOWN)) player_1_ddp -= 1700;
		}
		else {
		...
		if (is_down(BUTTON_Z) || is_down(BUTTON_W)) player_2_ddp += 1700;
		if (is_down(BUTTON_S)) player_2_ddp -= 1700;
		...
		// Rendering
		draw_rect(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 0xED7F10);
		...
		if (hot_button == 0) {
			draw_text("SINGLEPLAYER", -70, -10, 1, 0xFF0000);
			draw_text("MULTIPLAYER", 10, -10, 1, 0x00FF00);
		}
		else {
